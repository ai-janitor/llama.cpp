...
int ggml_metal_op_flash_attn_ext(ggml_metal_op_t ctx, int idx) {
    ...
    // Only for non-vec cases (nb>1); vec kernel handles nb=1 via simdgroup_reduction
    if (!props_dev->has_simdgroup_mm && !ggml_metal_op_flash_attn_ext_use_vec(op)) {
        const int dk = ne00;
        const int dv = ne00;
        ...
        // MLX-style: 1 query per threadgroup, BN=4 simdgroups × BD=32 threads
        const int BN = 4;
        const int BD = 32;
        const size_t smem = (2 * BN + BN * BD) * sizeof(float);
